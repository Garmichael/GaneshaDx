...
public static class GuiWindowTextureElement {
	...
	private static readonly List<Polygon> HoveredPolygons = new();
	private static readonly List<Vector2> HoveredVertices = new();

	private static Vector3 CameraPosition => new(
	...
	private static Vector2 PanningOffset => new(
		TextureSize / 2f - CameraSize + _cameraCenter.X,
		TextureSize / 2f - CameraSize + _cameraCenter.Y
	);

	private static Vector2 AdjustedMousePosition => new(
	...
	private static Vector2 PixelMousePosition => new(
	...
	private static void SetPolygonEffectForTexture() {
		Vector4 neutralLight = new(0.5f, 0.5f, 0.5f, 1f);
	...
	private static VertexPositionNormalTexture[] BuildTextureRenderVertices() {
		VertexPositionNormalTexture[] textureVerts = { new(), new(), new(), new() };
