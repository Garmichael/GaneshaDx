...
public class RotationWidgetAxis {
	...
	private readonly List<Vector3> _axisVertices = new();
	private VertexPositionColorTexture[] _axisPoleRenderVertices = Array.Empty<VertexPositionColorTexture>();

	private readonly List<Vector3> _rotationButtonDefinition;
	private readonly List<Vector3> _leftButtonVertices = new();
	private readonly List<Vector3> _rightButtonVertices = new();
	private VertexPositionColorTexture[] _leftButtonRenderVertices = Array.Empty<VertexPositionColorTexture>();
	private VertexPositionColorTexture[] _rightButtonRenderVertices = Array.Empty<VertexPositionColorTexture>();
	...
	private List<Vector3> GetAxisArrowDefinition() {
		return new List<Vector3> {
			new(-DistanceFromCenter, -ArrowWidth, ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, -ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, -ArrowWidth, ArrowWidth),
			new(-DistanceFromCenter - ArrowLength, ArrowWidth, ArrowWidth),
			new(-DistanceFromCenter - ArrowLength, ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, -ArrowWidth),
			new(-DistanceFromCenter, ArrowWidth, ArrowWidth),
	...
	private List<Vector3> GetRotationButtonDefinition() {
		...
		return new List<Vector3> {
			//front
			new(frontPlaneX, topPlaneY, middlePlaneZ),
			new(frontPlaneX, bottomPlaneY, rightPlaneZ),
			new(frontPlaneX, bottomPlaneY, leftPlaneZ),

			//back
			new(backPlaneX, bottomPlaneY, leftPlaneZ),
			new(backPlaneX, bottomPlaneY, rightPlaneZ),
			new(backPlaneX, topPlaneY, middlePlaneZ),
