...
public static class TransformWidget {
	...
	public static readonly List<int> SelectedVertexIndices = new();
	public static Vertex SnappingVertex;
	...
	static TransformWidget() {
		AxesSet = new List<TransformWidgetAxis> { new(Axis.X), new(Axis.Y), new(Axis.Z) };
	}
	...
	private static void SetOnlyOneAxisToHovered() {
		List<TransformWidgetAxis> hoveredAxes = new();
		foreach (TransformWidgetAxis axis in AxesSet) {
			if (axis.IsHovered) {
	...
	public static void SelectNextVertex(bool reverse) {
		if (Selection.SelectedPolygons[0].IsQuad) {
			List<int> vertexOrder = new() { 0, 1, 3, 2 };
