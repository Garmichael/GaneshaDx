...
public class GnsResourceRow {
	...
	private void SetResourceType() {
		...
		List<byte> acceptableHeaders = new() {34, 48, 112};
		...
		Dictionary<byte, ResourceType> resourceTypes = new() {
		...
		if (high == 1 && resourceTypes.ContainsKey(low)) {
			...
			if (ResourceType == ResourceType.OverrideMeshData) {
				OverlayConsole.AddMessage("Uhm");
			}
			return;
		}
